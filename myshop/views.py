...
from myshop.models import Category ,Sub_Category ,Banner,Upload_data,Upload_images ,Product , Category_banner ,Size_quantity ,Profile
...
def category(request,post_slug):
    ...
    for info in object1:
        ...
        for info1 in product:
            data1.append({'product_name':info1.brand_name,'product_title':info1.title,'product_price':info1.price,'product_discount':info1.discount,
                          'product_slug':info1.slug,'image':Upload_images.objects.filter(image_id__id=info1.id)})


    return render(request,"category-1.html",{'data':data,'object':object,'data1':data1,'image_object':image_object,'product':product,'user':request.user})
...
def product_detail(request,post_slug):
    ...
    return render(request,"product-detail.html",{'data':data,'data1':data1,'user':request.user,'is_liked':is_liked,'object':object})
...
def sign_up(request):
    if request.method == "POST":
        ...
        if (userform.is_valid()) and (profileform.is_valid()):
            ...
            return render(request, "signup.html", {"userform": userform, "profileform": profileform, 'error': userform.errors})
